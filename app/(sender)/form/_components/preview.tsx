...
const Preview = () => {
    ...
    // Check if we should render the new template
    const shouldRenderTemplate = template && ['professional', 'playful', 'elegant'].includes(template);
    ...
    if (shouldRenderTemplate) {
        ...
        const messageData: NovaxMessageData = {
            ...
            icons: headerIcons.map((icon, index) => ({
                position: index + 1,
                iconSrc: icon.src,
        ...
        return (
            <section className='flex flex-col items-center gap-[50px] w-full max-w-4xl mx-auto'>
                <div className='mt-20 w-full'>
                    <div className='text-center mb-8'>
                        <h2 className='text-[32px] font-bold text-stone-800 mb-2'>Preview Your Message</h2>
                        <p className='text-stone-600'>This is how your recipient will see it</p>
                    </div>
                    
                    <div className='bg-white rounded-lg shadow-lg p-8 max-h-[600px] overflow-y-auto'>
                        <TemplateComponent data={messageData} customization={customization} />
                    </div>
                </div>
                ...
                <Button 
                    ...
                    disabled={!template} 
                    className='text-black mx-auto mb-[42px] hover:text-white w-[107px] bg-[#FFFFFF] py-[10px] px-5 flex items-center gap-[10px]'
                >
                    Enter 
                    <Image 
