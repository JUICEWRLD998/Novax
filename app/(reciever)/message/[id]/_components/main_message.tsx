...
const MainMessage = () => {
  ...
  const handleResponsePick = async (selectedChoice: "yes" | "no") => {
    try {
      // Since we don't have a database, we just store the response locally
      // and show success message. In a future version, this could update the URL hash
      // or send via email/webhook
      
      toast.success(`You selected ${selectedChoice === "yes" ? "Yes" : "No"}! 💕`);
      return true;
    } catch (error) {
